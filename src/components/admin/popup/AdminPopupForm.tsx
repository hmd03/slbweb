...
const AdminPopupForm: React.FC = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`api/popups?page=${pageIndex}`);

            const popupList = await Promise.all(
                response.data.popupList.map(async (popup: any) => {
                    const mediaSrc = await fetchFile(popup.media.id);
                    popup.media.src = mediaSrc;
                    return popup;
                })
            );
        ...
        <AdminCurrentLayout title='팝업 관리 리스트'>
            <div className='w-full h-fit p-5 border border-Black bg-White'>
                <div className={`flex width-full pb-6 gap-2 items-center`}>
                    <Button theme='admin' className='w-[5rem] h-[3rem] '>
                        전체
                    </Button>
                    <Button theme='admin' className='w-[5rem] h-[3rem] '>
                        PC
