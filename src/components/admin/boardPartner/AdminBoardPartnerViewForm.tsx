...
const AdminBoardPartnerViewForm: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (params.id) {
                try {
                    const response = await axios.get(`/api/partnership/proposals/${params.id}`);
    ...
    const deleteId = async () => {
        try {
            if (params.id) {
                setLoading(true);
                const response = await axios.delete(`api/partnership/proposals/${params.id}`);
            ...
            <div className='w-full h-fit p-5 border border-Black bg-White flex flex-col items-center justify-center'>
                <table className="min-w-full border-collapse border border-[2px] border-Black">
                    ...
                    <tbody className='text-diagram'>
                        ...
                        <tr >
                            <th className={thClassName}>담당자명/직함</th>
                            <td className={tdClassName}>
                                {item.sender}
                            </td>
                        </tr>
...
export default AdminBoardPartnerViewForm;
