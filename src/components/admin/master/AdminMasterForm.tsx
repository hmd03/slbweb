...
const AdminMasterForm: React.FC = () => {
  ...
  const { isSupervisor, id } = useRecoilValue(UserState);
  ...
  const handleModClick = (idValue: string, itemSupervisor: boolean) => {
    if (!isSupervisor && idValue != id) {
      handleOpenModal('사용할 수 없는 기능입니다.', false, handleCancel);
      return;
    }
    navigate(`/admin/master/write/no/${idValue}/${itemSupervisor ? 1 : 0}`);
  };
