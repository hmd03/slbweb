...
const InquiryForm: React.FC = () => {
  ...
  const onSubmit = async () => {
    ...
    const discoveryRoute = selectedSource === '기타' ? etcSourceRef.current?.value : selectedSource;

    // 추후 조건 설정
    if (true || !senderName || !senderContact || !preferredRegion || !ageGroup || !discoveryRoute || !content || !agreed) {
  ...
  const handleConfirm = async () => {
    try {
      ...
      const payload = {
        ...
        discoveryRoute: selectedSource === '기타' ? etcSourceRef.current?.value : selectedSource,
  ...
  const tableCellStyle = 'border border-black p-2 align-top text-detail';
  const tableHeadStyle = 'bg-LightGray font-semibold text-left whitespace-nowrap w-[30%]';
    ...
    <div className="w-full flex justify-center px-4 py-8 bg-white">
      <div className="w-full max-w-[1300px]">
        <div className="flex flex-col items-center justify-center mb-6">
          ...
          <div className="w-[1px] h-10 border-r border-black my-5" />
          ...
          <tbody>
            <tr>
              ...
              <td className={tableCellStyle}>
                <input ref={nameRef} type="text" placeholder="이름을 입력하세요" className="w-full border border-gray-300 p-2" />
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <th className={`${tableCellStyle} ${tableHeadStyle}`}>✓ 창업 희망 지역</th>
              <td className={tableCellStyle}>
                <input ref={regionRef} type="text" placeholder="예: 서울 강남구" className="w-full border border-gray-300 p-2" />
              </td>
            </tr>
            <tr>
              <th className={`${tableCellStyle} ${tableHeadStyle}`}>✓ 연령대</th>
              <td className={tableCellStyle}>
                <select ref={ageRef} className="w-full border border-gray-300 p-2">
            ...
            <tr>
              <th className={`${tableCellStyle} ${tableHeadStyle}`}>✓ SLB를 최초 알게 된 경로</th>
              <td className={`${tableCellStyle} space-y-1`}>
                    ...
                    <div key={idx} className="flex items-center">
                      ...
                      {label === '기타' && selectedSource === '기타' && (
                        <input
                          ref={etcSourceRef}
                          type="text"
                          className="ml-2 border border-gray-300 p-1 flex-1"
                          placeholder="기타 입력"
                        />
                      )}
                    </div>
        ...
        {/* 개인정보 동의 */}
        <div className="mt-4">
          <label className="flex items-center text-detail">
