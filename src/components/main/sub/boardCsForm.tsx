...
const BoardCsForm: React.FC = () => {
  ...
  const onSubmit = async () => {
    ...
    if (!categoryId || !name || !phone1Ref.current?.value || !phone2Ref.current?.value || !phone3Ref.current?.value || !content || !agreed) {
  ...
  const handleConfirm = async () => {
    try {
      ...
      formData.append('senderContact', `${phone1Ref.current?.value}-${phone2Ref.current?.value}-${phone3Ref.current?.value}`);
      formData.append('senderEmail', `${emailIdRef.current?.value}@${emailDomainRef.current?.value}`);
