...
const BoardPartnerForm: React.FC = () => {
  ...
  const onSubmit = async () => {
    ...
    if ((emailLocal != '' || emailDomain != '') && !isValidEmail(emailLocal, emailDomain)) {
      handleOpenModal('유효한 이메일 형식을 입력해 주세요.', false, handleCancel);
      return;
    }

    if (!title || !manager || !phone1Ref.current?.value || !phone2Ref.current?.value || !phone3Ref.current?.value || !content || !agreed) {
      handleOpenModal('필수 항목을 모두 입력해 주세요.', false, handleCancel);
      return;
    }
    ...
    <div className="w-full flex justify-center px-4 py-8 bg-white">
      <div className="w-full max-w-[1300px]">
        ...
        <table className="w-full table-fixed border border-black border-separate border-spacing-0 text-detail">
          ...
          <tbody>
            <tr>
              <th className={`${tableCellStyle} ${tableHeadStyle}`}>✓ 제목</th>
              <td className={tableCellStyle}><input ref={titleRef} className="w-full border border-gray-300 p-2" /></td>
            </tr>
            <tr>
              <th className={`${tableCellStyle} ${tableHeadStyle}`}>회사명</th>
              <td className={tableCellStyle}><input ref={companyRef} className="w-full border border-gray-300 p-2" /></td>
            </tr>
            <tr>
              <th className={`${tableCellStyle} ${tableHeadStyle}`}>✓ 담당자명 / 직함</th>
              <td className={tableCellStyle}><input ref={managerRef} placeholder="담당자명/직함" className="w-full border border-gray-300 p-2" /></td>
            </tr>
            <tr>
            ...
            <tr>
              <th className={`${tableCellStyle} ${tableHeadStyle}`}>✓ 제안 내용</th>
              <td className={tableCellStyle}>
                <textarea ref={contentRef} className="w-full h-40 border border-gray-300 p-2 resize-none" placeholder="제안 내용을 입력해 주세요" />
              </td>
            </tr>
          </tbody>
        </table>
