...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    const widths = menuRefs.current.map(ref => ref ? ref.offsetWidth : 0);
    console.log(widths);
    setMenuWidth(widths);
  }, [isMenuOpen]);

  const mainMenuItems = [
    { title: '한식X샐러드&포케 SLB?', link: '/link1' },
    { title: '기회를 붙잡는 노하우', link: '/link2' },
    { title: '끝까지 함께 성장합니다!', link: '/link3' },
    { title: '성장하는 SLB 개설비용', link: '/link4' },
    { title: '성공창업문의', link: '/link5' },
    { title: 'SLB 이야기', link: '/link6' },
  ];

  const subMenuItems = [
    [
      { title: '맛에 대한 고집이/n남다른 특별함을 만든다', link: '/sub1-1' },
      { title: '아쉬움에 항상 고민합니다', link: '/sub1-2' },
      { title: '동상이몽 창업?', link: '/sub1-3' },
      { title: '배달부터 홀 운영까지/n해본 사람들이 함계합니다', link: '/sub1-4' },
    ],
    [
      { title: '성장하는 시장/n성장하는 아이템 SLB', link: '/sub2-1' },
      { title: '고객이 인정하는/n샐러드&포케 맛집!', link: '/sub2-2' },
      { title: '음식점 일이/n이렇게 쉬워도 되나요?', link: '/sub2-3' },
      { title: '특별한 메뉴소개', link: '/sub2-4' },
    ],
    [
      { title: 'SLB는 점주님과/n끝까지 합께 합니다', link: '/sub3-1' },
      { title: '청년, 중장년, 남녀/n누구나 성공창업가능!', link: '/sub3-2' },
      { title: '샐러드&포케 창업/n이것만은 꼭! 확인하세요?', link: '/sub3-3' },
      { title: '고객이 묻습니다./n카페인가요? 힙한 인테리어', link: '/sub3-4' },
    ],
    [
      { title: '어떤 상권에서도/n안정적인 수익성!', link: '/sub4-1' },
      { title: '과하지 않은/n맞춤 창업 비용', link: '/sub4-2' },
      { title: '꼼꼼하고 체계적인/nSLB 창업절차', link: '/sub4-3' },
      { title: '일잘하는 사람들의/n운영관리 시스템', link: '/sub4-4' },
    ],
    [
      { title: '온라인 창업문의', link: '/sub5-1' },
      { title: '고객문의', link: '/sub5-2' },
      { title: '협력제안', link: '/sub5-3' },
      { title: '찾아오시는 길', link: '/sub5-4' },
    ],
    [
      { title: '브랜드 스토리', link: '/sub6-1' },
      { title: '공지&뉴스', link: '/sub6-2' },
      { title: '샐톡톡 이벤트', link: '/sub6-3' },
      { title: '매장안내', link: '/sub6-4' },
    ]
  ];
  ...
  return (
    <header className="bg-white shadow w-full">
      ...
      <nav className="relative text-center" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
        ...
        {isMenuOpen && (
          <div className="py-6 absolute left-0 w-full bg-black bg-opacity-70 text-white rounded-md">
